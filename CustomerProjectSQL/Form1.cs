...
namespace CustomerProjectSQL
{
    public partial class Form1 : Form
    {
        ...
        private string GetCustomer()
        {
            ...
            try
            {
                (var dt, var err) = clsGetMethods.GetCustomerByQuery();
                errRes = err;
        ...
        private string GetCustomerById(int custId)
        {
            var errRes = string.Empty;
            try
            {
                (var dt, var err) = clsGetMethods.GetCustomerByFunction(custId);
                errRes = err;
                if (string.IsNullOrWhiteSpace(errRes))
                    GridFill(dt);
            }
        ...
        private string GetCustomerFunction()
        {
            var errRes = string.Empty;
            try
            {
                (var dt, var err) = clsGetMethods.GetCustomerByFunction();
                errRes = err;
                if (string.IsNullOrWhiteSpace(errRes))
                    GridFill(dt);
            }
        ...
        private string GetGender()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(errRes))
                {
                    cmbGender.ValueMember = dt.Columns["IDN"].ToString(); //IDN
        ...
        private string GetCustomer(ClassControls cls)
        {
            ...
            try
            {
                (var dt, var err) = clsGetMethods.GetCustomerByView(cls);
                errRes = err;
                if (string.IsNullOrWhiteSpace(errRes))
                    GridFill(dt);
            }
        ...
        private void btnSearchById_Click(object sender, EventArgs e)
        {
            ...
            var custId = int.Parse(txtCustIdn.Text);
            var err = GetCustomerById(custId);
            if (!string.IsNullOrWhiteSpace(err))
            {
                MessageBox.Show(err);
                return;
